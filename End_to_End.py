...
def traitement_plaque(image_uploaded):
    def load_model(path):
        try:
    ...
    test_image_path = "tempDir/"+image_uploaded
    ...
    print(final_string)
    st.success("La plaque d'immatriculation est : " + final_string)
    #plt.savefig('final_result.png', dpi=300)


image_file = st.file_uploader("Choisissez une image", type=['png', 'jpeg', 'jpg'])
if image_file is not None:
    ...
    with open(os.path.join("tempDir", image_file.name), "wb") as f:
        test = f.write(image_file.getbuffer())
    st.success("Saved File")
    traitement_plaque(image_file.name)
